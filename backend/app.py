...
def create_app():
    ...
    def create_wallet(
        wallet: schemas.WalletCreate, db: Session = Depends(database.get_db), current_user: schemas.User = Depends(get_current_user)
    ):
        return crud.create_wallet(db=db, wallet=wallet, user_id=current_user.id)

    @app.get("/wallets/", response_model=list[schemas.Wallet])
    def read_wallets(
        skip: int = 0, limit: int = 100, db: Session = Depends(database.get_db), current_user: schemas.User = Depends(get_current_user)
    ):
        wallets = crud.get_wallets(db, user_id=current_user.id, skip=skip, limit=limit)
        return wallets

    @app.get("/hyperliquid/meta")
    def get_hyperliquid_meta(hl_api: HyperliquidAPI = Depends(HyperliquidAPI)):
    ...
    def create_bot(
        bot: schemas.BotCreate, db: Session = Depends(database.get_db), current_user: schemas.User = Depends(get_current_user)
    ):
    ...
    def place_order(
        wallet_id: int, order: schemas.OrderRequest, db: Session = Depends(database.get_db), current_user: schemas.User = Depends(get_current_user)
    ):
    ...
    def get_open_orders(
        wallet_id: int, db: Session = Depends(database.get_db), current_user: schemas.User = Depends(get_current_user)
    ):
        wallet = crud.get_wallet(db, wallet_id=wallet_id, user_id=current_user.id)
        if not wallet:
            raise HTTPException(status_code=404, detail="Wallet not found")
    ...
    def run_bot(
        bot_id: int, run_request: schemas.BotRunRequest, db: Session = Depends(database.get_db), current_user: schemas.User = Depends(get_current_user)
    ):
    ...
    @app.post("/vaults/deposit")
    def vault_deposit(
        deposit_request: schemas.VaultDepositRequest, db: Session = Depends(database.get_db), current_user: schemas.User = Depends(get_current_user)
    ):
